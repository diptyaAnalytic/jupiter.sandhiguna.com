...
class OpenSSHClientGenerator(OpenSSHGenerator):
    ...
    def test_config(cls, config):
        ...
        if _pre_8_5_ssh():
            config = config.replace('PubkeyAcceptedAlgorithms',
...
class OpenSSHServerGenerator(OpenSSHGenerator):
    ...
    def _test_setup(cls):
        ...
        ret = 255
        try:
            ret = subprocess.call('/usr/bin/ssh-keygen -t rsa -b 2048 '
...
def _pre_8_5_ssh():
    try:
        ...
        ssh_version = subprocess.run(['/usr/bin/ssh', '-V'],
                                     stderr=subprocess.PIPE).stderr.decode()
        ver = re.match(r'OpenSSH_(\d+).(\d+)p.*', ssh_version)
        return ver and tuple(int(n) for n in ver.groups()) < (8, 5)
